...
Vagrant.configure(2) do |config|
  ...
  config.vm.provider :virtualbox do |vb|
    vb.customize ["modifyvm", :id, "--cpus", "1", "--memory", "1024"] #best practice for vagrant is to use 1/4 of the host's memory
  ...
  #  sudo su - postgres -c "createdb selfdeploy_test"
   sudo apt-get install postgresql postgresql-contrib
   sudo -i -u postgres psql -c "ALTER ROLE vagrant WITH SUPERUSER;"
   sudo -i -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE selfdeploy_test TO vagrant;"

   #Install SD2 full stack
   cd /vagrant
   bundle install
